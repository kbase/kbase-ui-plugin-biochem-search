...
class ReactionTable extends Component {
    static aliasFormatter(cell, row) {
        return (
            <span>
                {String(cell)
                    .replace(/\|/g, ', ')
    ...
    static renderHalfRxn(compounds) {
        return compounds
            .split('+')
            .map((i) => {
                return ReactionTable.renderCompound(i);
            })
            .reduce((prev, curr) => [
                prev,
        ...
        return (
            <div className="container">
                <div className="row align-items-center">
                    {ReactionTable.renderHalfRxn(reactants)}
                    <h4 className="col-md-auto">{sign}</h4>
                </div>
                <div className="row align-items-center">{ReactionTable.renderHalfRxn(products)}</div>
            </div>
        );
    }
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    dataField: 'aliases',
                    text: 'Aliases',
                    formatter: ReactionTable.aliasFormatter
                }
            ]
        };
    }
